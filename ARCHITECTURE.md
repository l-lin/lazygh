...
`update_interaction.go` also now stops at typed shell commands for transient popup errors, open-link actions, and clipboard-preparation flows. Detail-search word lookup and follow-up scrolling now use the same `cmd_interaction.go` command surface, so `update_navigation_interaction.go` / `update.go` no longer resolve live detail views or call direct follow helpers inline inside the reducer. The same shell-command surface now also owns async popup error reporting, GUI reconfiguration after theme changes, and manual pull-request refresh registration, so `update_feature_async.go`, `update_actions_popup.go`, and `update_async.go` no longer branch on `program.gui` or direct popup helpers.
