...
func TestStatusLineKeyHints_GivenBuildRunPopupVisible_WhenRendering_ThenItShowsPopupHintsRightAlignedOnTheBottomRow(t *testing.T) {
	...
	then_statusLineKeyHintsAre(t, gui, "/: search, y: yank, Alt+Y: copy, Escape: back")
	then_viewLineSegmentHasForegroundColor(t, gui, viewStatusLineKeyHintsName, 0, "/: search, y: yank, Alt+Y: copy, Escape: back", given_themeColorHex(t, theme.InactiveTitleHex), "build run popup key hints")
	then_statusLineKeyHintsAreRightAligned(t, gui, "/: search, y: yank, Alt+Y: copy, Escape: back")
}
