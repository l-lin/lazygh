...
func TestReviewMode_GivenTheMetadataPaneSelected_WhenRendering_ThenViewZeroShowsThePullRequestDescription(t *testing.T) {
	...
	for _, hidden := range []string{"1 reviewer has not approved yet.", "No status checks reported.", "No builds reported."} {
		if strings.Contains(detailView.Buffer(), hidden) {
			t.Fatalf("expected the still-folded merge-check and build overview sections to hide %q, actual %q", hidden, detailView.Buffer())
		}
	}
	...
	if !strings.Contains(detailView.Buffer(), "Created by") || !strings.Contains(detailView.Buffer(), " "+pullRequestOverviewPendingIcon+" Reviewers (0/1)") || !strings.Contains(detailView.Buffer(), "@reviewer-requested") || !strings.Contains(detailView.Buffer(), "Rendered body 42") {
		t.Fatalf("expected the review detail pane to keep showing the rendered overview with the open reviewer section, actual %q", detailView.Buffer())
	}
...
func TestReviewMode_GivenTheDescriptionOverviewSections_WhenPressingEnter_ThenItTogglesTheSelectedSection(t *testing.T) {
	...
	if !strings.Contains(detailView.Buffer(), " "+pullRequestOverviewPendingIcon+" Reviewers (0/1)") {
		t.Fatalf("expected enter to collapse the review overview section, actual %q", detailView.Buffer())
	}
	...
	if !strings.Contains(detailView.Buffer(), " "+pullRequestOverviewPendingIcon+" Reviewers (0/1)") || !strings.Contains(detailView.Buffer(), "@reviewer-requested") {
		t.Fatalf("expected enter to expand the review overview section again, actual %q", detailView.Buffer())
	}
}
...
func TestReviewMode_GivenTheSelectedFileDiff_WhenRendering_ThenViewZeroUsesDiffColorsForCountsHunksAndChangedLines(t *testing.T) {
	...
	then_viewLineSegmentHasForegroundColor(t, gui, viewDetailName, deletionLineIndex, "old line", given_themeColorHex(t, theme.ActiveTextHex), "review diff deletion text")
	...
	then_viewLineSegmentHasForegroundColor(t, gui, viewDetailName, additionLineIndex, "new line", given_themeColorHex(t, theme.ActiveTextHex), "review diff addition text")
...
func TestReviewMode_GivenAnExpandedInlineConversation_WhenRendering_ThenItShowsTheThreadChevronAndTheLocationWithoutASideAnchor(t *testing.T) {
	...
	headerLineIndex := given_viewLineIndexContaining(t, detailView, " internal/tui/render.go:3")
	if strings.Contains(detailView.BufferLines()[headerLineIndex], "R3") || strings.Contains(detailView.BufferLines()[headerLineIndex], "L3") {
		t.Fatalf("expected the conversation header to drop the side anchor, actual %q", detailView.BufferLines()[headerLineIndex])
	}
...
func TestReviewMode_GivenAResolvedInlineConversation_WhenRendering_ThenItStartsCollapsedAndDropsTheSideAnchor(t *testing.T) {
	diff := given_reviewSessionPullRequestDiff()
	diff.Threads = []githubcli.PullRequestReviewThread{{
		ID:           "thread-1",
		IsResolved:   true,
	...
	detailView, actualErr := gui.View(viewDetailName)
	then_noError(t, actualErr)
	headerLineIndex := given_viewLineIndexContaining(t, detailView, " internal/tui/render.go:2")
	if strings.Contains(detailView.BufferLines()[headerLineIndex], "L2") || strings.Contains(detailView.BufferLines()[headerLineIndex], "R2") {
		t.Fatalf("expected the collapsed conversation header to drop the side anchor, actual %q", detailView.BufferLines()[headerLineIndex])
	}
	if strings.Contains(detailView.BufferLines()[headerLineIndex], "│") {
...
func TestReviewMode_GivenTheCursorOnAnInlineConversation_WhenPressingEnter_ThenItTogglesTheConversationVisibility(t *testing.T) {
	...
	actualErr = toggleHandler(gui, detailView)
	then_noError(t, actualErr)
	if !strings.Contains(detailView.Buffer(), " internal/tui/render.go:3") || !strings.Contains(detailView.Buffer(), "Unresolved") {
		t.Fatalf("expected the thread to collapse after pressing enter, actual %q", detailView.Buffer())
	}
	if strings.Contains(detailView.Buffer(), "R3") || strings.Contains(detailView.Buffer(), "L3") {
		t.Fatalf("expected the collapsed thread header to drop the side anchor, actual %q", detailView.Buffer())
	}
	if strings.Contains(detailView.Buffer(), "Rendered thread body") {
...
func TestReviewMode_GivenTheCursorOnAnInlineConversation_WhenPressingZA_ThenItTogglesTheConversationVisibility(t *testing.T) {
	...
	if strings.Contains(detailView.Buffer(), "R3") || strings.Contains(detailView.Buffer(), "L3") {
		t.Fatalf("expected za to keep the side anchor out of the header, actual %q", detailView.Buffer())
	}
	if strings.Contains(detailView.Buffer(), "Rendered thread body") {
...
func TestReviewMode_GivenTheCursorOnAResolvedCollapsedInlineConversation_WhenPressingEnter_ThenItExpandsAndRehidesTheConversation(t *testing.T) {
	...
	actualErr = toggleHandler(gui, detailView)
	then_noError(t, actualErr)
	if !strings.Contains(detailView.Buffer(), " internal/tui/render.go:3") || !strings.Contains(detailView.Buffer(), "Resolved") {
		t.Fatalf("expected the resolved thread to expand after pressing enter, actual %q", detailView.Buffer())
	}
	if strings.Contains(detailView.Buffer(), "R3") || strings.Contains(detailView.Buffer(), "L3") {
		t.Fatalf("expected the resolved thread header to drop the side anchor, actual %q", detailView.Buffer())
	}
	if !strings.Contains(detailView.Buffer(), "Rendered thread body") {
...
func TestReviewMode_GivenADirectoryRowInTheFilesPane_WhenPressingEnter_ThenItOpensViewZeroWithoutTogglingTheDirectory(t *testing.T) {
	loader := &fakePullRequestDetailLoader{
		startReviewID: "PRR_pending",
		diffs: map[string]githubcli.PullRequestDiff{
			"acme/widgets#42": given_reviewSessionPullRequestDiff(),
	...
	if !strings.Contains(filesView.Buffer(), " "+reviewDiffDirectoryIcon+" internal/tui/") {
		t.Fatalf("expected enter to keep the directory expanded, actual %q", filesView.Buffer())
	}
	for _, expected := range []string{"render.go", "model.go"} {
		if !strings.Contains(filesView.Buffer(), expected) {
			t.Fatalf("expected enter to keep %q visible in the file tree, actual %q", expected, filesView.Buffer())
		}
	}
}

func TestReviewMode_GivenTheSelectedFileIsInsideADirectory_WhenPressingZA_ThenItTogglesTheContainingDirectory(t *testing.T) {
	loader := &fakePullRequestDetailLoader{
		startReviewID: "PRR_pending",
		diffs: map[string]githubcli.PullRequestDiff{
			"acme/widgets#42": given_reviewSessionPullRequestDiff(),
	...
	if !strings.Contains(filesView.Buffer(), "render.go") || !strings.Contains(filesView.Buffer(), "model.go") {
		t.Fatalf("expected za to reopen the containing directory, actual %q", filesView.Buffer())
	}
