...
func (program *Program) applyActionsPopupRequestedMessage(requested Msg) []Cmd {
	...
	case MsgPullRequestMergeWhenReadyRequested:
		return program.applyPullRequestMergeWhenReadyRequested(actual)
...
func (program *Program) mutateLoadedPullRequestSummaries(identity githubdomain.PullRequest, mutate func(*githubdomain.PullRequest)) {
	...
	program.updatePastedPullRequestTabState(func(state pastedPullRequestTabState) pastedPullRequestTabState {
		...
		updatedPullRequests := append([]githubdomain.PullRequest(nil), state.pullRequests...)
		updated := false
		for index, summary := range updatedPullRequests {
			if !samePullRequestIdentity(summary, identity) {
				continue
			}
			mutate(&summary)
			updatedPullRequests[index] = summary
			updated = true
		}
		if !updated {
			return state
