...
func TestSharedKeybindingDefinitionFor_GivenMutualizedActions_WhenLookingThemUp_ThenItReturnsTheirBehaviorFirstScopesAndDefaults(t *testing.T) {
	testCases := []struct {
		...
		{name: "open pull request in browser", action: "open_pull_request_in_browser", expectedScope: keymapScopePullRequests, expectedBindings: []string{"alt+b"}, expectedAllowSequences: true},
		...
		{name: "move cursor to other end", action: "move_cursor_to_other_end", expectedScope: keymapScopeCursor, expectedBindings: []string{"o"}, expectedAllowSequences: false},
	}
